...
+ (NSDictionary *)layoutRelationDescriptionsByValue {
    ...
    static NSDictionary *descriptionZNKMAp;
    dispatch_once(&once, ^{
        descriptionZNKMAp = @{
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(ZNKMAs_key)] && [obj ZNKMAs_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj ZNKMAs_key]];
    }
